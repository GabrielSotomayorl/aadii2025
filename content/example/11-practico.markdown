...
# --- Inicio: Código comentado para instalar OpenMx si semPlot da problemas con dependencias ---
# # Si hay problemas para instalar semPlot debido a OpenMx en Windows R >= 4.2:
# # 1) URL del binario para la rama de R correspondiente (ej. 4.4)
# # Debes verificar la URL correcta para tu versión de R y OpenMx si este método es necesario.
# # mx_url <- "https://cran.r-project.org/bin/windows/contrib/4.4/OpenMx_2.21.13.zip" 
# 
# # 2) Instalar sin compilar desde la URL
# # install.packages(mx_url,
# #                  repos = NULL,
# #                  type  = "win.binary")
# --- Fin: Código comentado para OpenMx ---

library(haven)
library(lavaan)
library(dplyr)
library(semPlot) # Para graficar modelos SEM
...
**Interpretación del Gráfico:** El diagrama ahora muestra el path directo de `derecha` a `castigo_media` (con un coeficiente estandarizado de 0.055) además de las relaciones del modelo original. Los valores en las flechas son los coeficientes estandarizados (`Std.all`). Las varianzas residuales (1 - `\(R^2\)`) para `castigo_media` y `rwa_media` también se pueden inferir o mostrar directamente en el gráfico.
